...
.option('--quantity <quantity>', 'used in conjunction with --price and --salename and --pricepoint to set a sale rule. Type --salerule for more help')
.option('--salename <salename>', 'used in conjunction with --price and --quantity and --pricepoint to set a sale rule type --salerule for more help')
.option('--pricepoint <amount>', 'used in conjunction with --price, --quantity and --salename to set a sale rule type --salerule for more help')
.option('--salerule', `to create a new sale rule input the following commands- example: --price apple --quantity 3 --salename Threepak --pricepoint .75. This would create a 3 for .75 sale rule for apples.`)
...
if (program.salerule) {
  console.log(`to create a new sale rule input the following commands- example: ${chalk.yellow('--price')} apple  ${chalk.yellow('--quantity')} 3  ${chalk.yellow('--salename')} Threepak ${chalk.yellow('--pricepoint')} .75. 
This would create a 3 for .75 sale rule for apples`)
}

if(program.price && program.quantity && program.salename && program.pricepoint){
  console.log(program.price, program.quantity, program.salename, program.pricepoint)
}
